...
class Solution2(Scene):
    def construct(self):
        ...
        z_label = MathTex("\\text{}z = 1 + 1j",font_size=40).next_to(z_dot,UP+RIGHT)
        ...
        z_label_exponentional = MathTex("\\text{$z$} = 1.4e^{j\pi/4}",font_size=32).next_to(z_dot,UP+RIGHT)
        self.play(ReplacementTransform(z_label,z_label_exponentional))
        self.wait(2)

        # ANIMATION: ARGUMENT
        l1 = Line(graph.coords_to_point(0,0),graph.coords_to_point(1,1)).set_color(BLUE).set_opacity(1)
        z_label_exponentional[0][2:5].set_color_by_gradient(BLUE)
        self.play(Write(l1), FadeIn(z_label_exponentional[0][2:5], scope = 5))
        self.wait(2)
        
        # ANIMATION: PHASE
        sector = Sector(arc_center=graph.coords_to_point(0,0), start_angle = 0, angle=PI/4-0.08, radius= 0.5, color=GREEN).set_opacity(0.5)
        ...
        z_label_exp2 = MathTex("\\text{$z$}^2{ = 2e}^{j\pi/2}",font_size=32).next_to(z_dot2,UP+LEFT)
        ...
        z_label_exp2[0][3].set_color_by_gradient(BLUE)
        self.play(Write(l3), FadeIn(z_label_exp2[0][3], scope = 5))
        self.wait(2)
        
        # ANIMATION: PHASE
        sector2 = Sector(arc_center=graph.coords_to_point(0,0), start_angle=0, angle=PI/2, radius=0.3, color=GREEN).set_opacity(0.7)
        z_label_exp2[0][6:9].set_color_by_gradient(GREEN)
        self.play(Write(sector2), FadeIn(z_label_exp2[0][6:9], scope = 5))
        self.wait(2)
        ...
        #The solution
        text8 = MathTex(
                    "\\text{In this example modulus of $z$ is}",
                    font_size=40
                ).move_to(ORIGIN+2.5*UP)
        
        z = MathTex("|z| = |\\sqrt{2}| > 1", color=WHITE,font_size=40).next_to(text8, DOWN)
        text9 = MathTex("\\text{which is bigger than 1.}",font_size=40).move_to(ORIGIN+0.5*UP)
        text10 = MathTex(
            "\\text{Because of that, }", "{|z|}^{k}","\\text{ will diverge.}",
            font_size=40
        ).move_to(ORIGIN+0.5*DOWN)
        z_k = MathTex("\\lim_{k \\rightarrow +\\infty}|z|^k = \\lim_{k \\rightarrow +\\infty}|\\sqrt{2}|^k \\rightarrow +\\infty", color=WHITE,font_size=40).next_to(text10, DOWN)
        ...
        z_dot = Dot(graph.coords_to_point(1, 1), color=WHITE)
        lines = graph.get_lines_to_point(graph.c2p(1,1),color = DARK_GRAY)
